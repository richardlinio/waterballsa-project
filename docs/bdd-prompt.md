...
| Prompt       | 輸入                       | 輸出        | 用途                         |
| ------------ | -------------------------- | ----------- | ---------------------------- |
| **Prompt 1** | Release Spec               | DSL Feature | 業務層測試場景（給人看）     |
| **Prompt 2** | DSL + API Spec + DB Schema | ISA Feature | 技術層可執行測試（給機器跑） |
| **Prompt 3** | 測試失敗訊息               | 修正建議    | 迭代精煉達到 99% 精準度      |

### 核心理念

1. **分層抽象**: DSL 使用業務語言，ISA 使用技術語言
2. **原子化測試**: 每個步驟只做一件事，驗證分層
3. **通用步驟**: 使用可重用的 Step Definitions，不發明新步驟
4. **資料庫直接插入**: Setup 使用 `Given the database has a ...`，不透過 API
...
## Prompt 2: DSL → ISA Feature (技術層測試實作生成)

### 使用時機

當你有 DSL Feature，需要生成可執行的 ISA Feature。
...
````markdown
你是一位專業的 Backend 工程師，負責將業務測試場景翻譯為可執行的 API 整合測試。

## 你的任務

將以下 DSL Feature 翻譯為 ISA Feature 檔案，使其可以透過 Cucumber 執行。
...
1. Setup 使用 `the database has a ...` 步驟
2. 驗證分層且原子化 (HTTP → 結構 → 值)
3. 加入清楚的註解說明驗證層級
4. JSON 格式正確縮排 (2 空格)
5. 只使用已有的 Step Definitions
6. 確保測試資料獨立於其他測試
