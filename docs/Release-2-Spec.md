...
- 訂單狀態有兩種：
  - **未付款**：訂單已建立但尚未付款
  - **已付款**：付款完成，使用者擁有課程存取權
  - **已過期**：三天後未付款的訂單過期，無法付款，但使用者還是可以看的到
...
4. 若使用者無法刪訂單，訂單有有效期限嗎？例如三天過期？
5. 過期後的訂單，使用者還看的到嗎？或是就直接不顯示給使用者？
6. 使用者購買後，是否有觀看期限？
7. 使用者是否可重複購買相同課程？或是買給其他使用者？
8. 現階段是否需要折扣碼功能？例如參加過水球老師演講的可獲得八折折扣碼

關於待確認問題：

訂單管理：使用者只能選擇付款或放著不理，不能刪除訂單。訂單有效期限是三天，過期後訂單狀態會變成失敗，但使用者還是看得到。

觀看期限：使用者購買後沒有觀看期限，可以一直看。

重複購買：使用者不能重複購買相同課程，也不能買給其他使用者。

折扣碼功能：現階段不需要折扣碼功能喔，折價券會在付款頁面中顯示可以使用的畫面。
